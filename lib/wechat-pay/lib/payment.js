...
var URLS = {
  ...
  CREDIT_CARD: 'https://api.mch.weixin.qq.com/pay/micropay', //刷卡支付
};
...
// 扫码支付
Payment.prototype.scanCodePayment = function(order, callback) {
  var self = this;
  var default_params = {
  ...
  this._signedQuery(URLS.CREDIT_CARD, order, {
    https: true,
    required: requiredData
  }, callback);
...
Payment.prototype._httpsRequest = function(url, data, callback, donetPfx) {
...
Payment.prototype._signedQuery = function(url, params, options, callback, donetPfx) {
  var self = this;
...
Payment.prototype.validate = function(xml, callback) {
  ...
  xml2js.parseString(xml, {
    ...
    if (data.return_code == RETURN_CODES.FAIL) {
      ...
      error.info = 'InvalidSubMchId'
    } else if (data.sign && self._getSign(data) !== data.sign) {
      error = new Error();
      error.name = 'InvalidSignature';
      error.info = 'InvalidSignature'
    }
